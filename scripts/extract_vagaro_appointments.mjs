#!/usr/bin/env node
/**
 * Extract Vagaro Appointments/Services Report via authenticated Chromium daemon.
 * Navigates to Reports > Appointments and intercepts the API response.
 */
...
const REPORT_URL = 'https://us04.vagaro.com/merchants/reports/appointments/list';
const OUTPUT_DIR = process.env.VAGARO_EXPORT_DIR || path.resolve('data/incoming');
const OUTPUT_PATH = path.join(OUTPUT_DIR, 'vagaro_appointments_latest.json');
const REPORT_ENDPOINT = '/merchants/reports/getappointments';
...
function nextReportResponse(cdp) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => { cleanup(); reject(new Error('Timeout waiting for appointments response.')); }, 60_000);
...
async function main() {
  ...
  try {
    ...
    if (!Array.isArray(firstPage.Data)) throw new Error('Invalid appointments report response.');
    ...
    fs.writeFileSync(OUTPUT_PATH, JSON.stringify({
      source: 'vagaro-appointments-report',
...
main().catch(e => { process.stderr.write(`Appointments export failed: ${e.message}\n`); process.exit(1); });
