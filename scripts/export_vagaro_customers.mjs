#!/usr/bin/env node
/**
 * Export the full authenticated Vagaro Customer Report using the report UI's
 * own pagination requests. Customer data stays in the private ignored data
 * directory; stdout contains only sanitized counts and paths.
 */
...
function nextReportResponse(cdp) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      cleanup();
      reject(new Error('Timed out waiting for the Vagaro customer report response.'));
    }, 60_000);
...
async function main() {
  ...
  if (!page) throw new Error('No browser page is available. Vagaro re-login is required.');
  ...
  try {
    ...
    if (!Array.isArray(firstPage.Data)) {
      throw new Error(`Vagaro returned an invalid customer report response: ${JSON.stringify({
        status: firstPage.Status,
        keys: Object.keys(firstPage),
        dataType: typeof firstPage.Data,
      })}`);
    }
    ...
    for (let pageNumber = 2; pageNumber <= pageCount; pageNumber += 1) {
      ...
      await page.locator('.pagination-controls-left button').filter({ has: page.locator('i.fa-angle-right') }).evaluate(
        (button) => button.click(),
      );
      const response = await responsePromise;
      if (!Array.isArray(response.Data)) {
        throw new Error(`Vagaro customer report page ${pageNumber} was invalid.`);
      }
      rows.push(...response.Data);
    }

    const deduplicated = [...new Map(rows.map((row) => [row.UserID, row])).values()];
    if (deduplicated.length !== totalRecords) {
      throw new Error(`Customer record count mismatch: expected ${totalRecords}, received ${deduplicated.length}.`);
    }
    ...
    fs.writeFileSync(temporaryPath, JSON.stringify(exportRecord), { mode: 0o600 });
...
main().catch((error) => {
  process.stderr.write(`Vagaro export failed: ${error.message}\n`);
  process.exit(1);
});
